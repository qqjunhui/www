...
class backshop extends indexMain {
    ...
    function addshop(){
        ...
        $senotes=addslashes($_POST["senotes"]);
        $srules=$_POST['srules'];
        $lid=$_POST['lid'];
        $rid=$_POST["rid"];
        //连接数据库插入
//        echo $uid."<br>".$simg."<br>".$sname."<br>".$sename."<br>".$snotes."<br>".$saddress."<br>".$senotes."<br>".$srules."<br>".$lid."<br>".$rid."<br>";
//        exit();
        $db=new db2("shop");
        $result=$db->insert("sename='{$sename}',simg='{$simg}',sname='{$sname}',snotes='{$snotes}',saddress='{$saddress}',senotes='{$senotes}',srules='{$srules}',lid='{$lid}',uid='{$uid}',rid='{$rid}',srec=0,status=1,state=0");
        if($result>0){
            $db2=new db2("user");
    ...
    function showcom(){
        ...
        $db2=new db2("slist");
        $slist=$db2->where("sid=".$sid)->select();
        if ($slist){
            $this->smarty->assign("slist",$slist);
        }
