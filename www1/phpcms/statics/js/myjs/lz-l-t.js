...
window.onload=function(){
    function xuanka(xuanka1, xuanka2) {
        let plp = document.querySelectorAll(xuanka1);
        let blb = document.querySelectorAll(xuanka2);
        for (let i = 0; i < plp.length; i++) {
            plp[i].onmouseover = function () {
                for (let j = 0; j < blb.length; j++) {
                    plp[j].classList.remove("first");
                    blb[j].style.display = "none";
                }
                plp[i].classList.add("first");
                blb[i].style.display = "block";
            }
        }
    }

    xuanka('.ljb-qqq p', '.olm');
