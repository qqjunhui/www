		...
		if(isset($_GET['dosubmit'])) {
			...
			foreach ($types as $_k=>$_v) {
				if($key==$_k) {
					...
					$page++;
					if($pages>=$page) showmessage("正在更新 <span style='color:#ff0000;fonts-size:14px;text-decoration:underline;' >{$_v['name']}</span> - 总数：{$total} - 当前第 <fonts color='red'>{$page}</fonts> 页","?m=search&c=search_admin&a=createindex&menuid=909&page={$page}&total={$total}&key={$key}&pagesize={$pagesize}&have_truncate=1&dosubmit=1");
					$key++;
					showmessage("开始更新： <span style='color:#ff0000;fonts-size:14px;text-decoration:underline;' >{$_v['name']}</span> - 总数：{$total}条","?m=search&c=search_admin&a=createindex&menuid=909&page=1&key={$key}&pagesize={$pagesize}&have_truncate=1&dosubmit=1");
