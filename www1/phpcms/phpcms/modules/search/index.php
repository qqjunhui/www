				...
				if($modelid) {
					...
					if(!empty($segment_q)) {
						...
						foreach($replace as $replace_arr_v) {
							$replace_arr[] =  '<fonts color=red>'.$replace_arr_v.'</fonts>';
						}
						...
						foreach($data as $_k=>$_v) {
							$data[$_k]['title'] = str_replace($q, '<fonts color=red>'.$q.'</fonts>', $_v['title']);
							$data[$_k]['description'] = str_replace($q, '<fonts color=red>'.$q.'</fonts>', $_v['description']);
						}
					}
				} else {
					//读取专辑搜索接口
