...
class special extends admin {
	...
	public function public_create_type() {
		...
		if (is_array($types) && !empty($types) || $pages) {
			...
			if ($pages<=$maxpage) {
				...
				showmessage($typename.L('type_from').($_GET['page'] ? $_GET['page'] : 1).L('type_end').$maxpage.'</fonts> '.L('update_success'), '?m=special&c=special&a=public_create_type&typeid='.$typeid.'&typename='.$typename.'&page='.$page.'&pages='.$pages.'&specialid='.$specialid);
			}
	...
	public function public_create_content() {
		...
		if ($page>=$pages) {
			...
			$page++;
			showmessage(L('content_from').' <fonts color="red">'.intval($offset+1).L('type_end').intval($offset+10).'</fonts> '.L('update_success'), '?m=special&c=special&a=public_create_content&specialid='.$specialid.'&page='.$page.'&pages='.$pages);
		}
	}
