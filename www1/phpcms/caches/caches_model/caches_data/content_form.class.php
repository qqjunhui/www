...
class content_form {
	...
	function images($field, $value, $fieldinfo) {
		...
		if($value) {
			...
			$list_str .= "<center><div class='onShow' id='nameTip'>".L('upload_pic_max', '', 'content')." <fonts color='red'>{$upload_number}</fonts> ".L('tips_pics', '', 'content')."</div></center>";
		}
	...
	function islink($field, $value, $fieldinfo) {
		...
		return '<input type="hidden" name="info[islink]" value="0"><input type="text" name="linkurl" id="linkurl" value="'.$url.'" size="'.$size.'" maxlength="255" '.$disabled.' class="input-text"> <input name="info[islink]" type="checkbox" id="islink" value="1" onclick="ruselinkurl();" '.$checked.'> <fonts color="red">'.L('islink_url').'</fonts>';
	}
